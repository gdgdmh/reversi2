...
// CPULEVELとみなす文字列パターン
const std::string
    reversi::KeyboardSelectCpuLevel::LEVEL_STRINGS[LEVEL_PATTERN_COUNT] = {
        "1", "2", "3", "4"};
 ...
 */
bool reversi::KeyboardSelectCpuLevel::IsSelected() { return isSelected; }

/**
 * Cpuレベルを取得する
 * @return cpuレベル
 */
int reversi::KeyboardSelectCpuLevel::GetCpuLevel() { return cpuLevel; }

/**
 * 間違った入力をしたか
 * @return trueなら間違った入力をした
 */
bool reversi::KeyboardSelectCpuLevel::IsWrongInput() { return isWrongInput; }
 ...
 */
bool reversi::KeyboardSelectCpuLevel::CheckInputStringLevel(
    const std::string& checkString, int& selectCpuLevel) {
  for (int i = 0; i < LEVEL_PATTERN_COUNT; ++i) {
    reversi::Assert::AssertArrayRange(
        i, LEVEL_PATTERN_COUNT,
        "KeyboardSelectCpuLevel::CheckInputStringLevel index over yes");
    // 文字列一致をチェック
