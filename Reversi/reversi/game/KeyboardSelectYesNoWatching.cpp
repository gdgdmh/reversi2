...
// 定数定義
// Yesとみなす大文字文字列パターン
const std::string reversi::KeyboardSelectYesNoWatching::YES_UPPER_STRINGS
    [YES_UPPER_PATTERN_COUNT] = {"Y", "YES"};
// Noとみなす大文字文字列パターン
const std::string reversi::KeyboardSelectYesNoWatching::NO_UPPER_STRINGS
    [NO_UPPER_PATTERN_COUNT] = {"N", "NO"};
// Watchingとみなす大文字文字列パターン
const std::string reversi::KeyboardSelectYesNoWatching::WATCHING_UPPER_STRINGS
    [NO_UPPER_PATTERN_COUNT] = {"W", "WATCHING"};
...
bool reversi::KeyboardSelectYesNoWatching::IsSelectWatching() {
  if (selectReply ==
      reversi::KeyboardSelectYesNoWatching::SELECT_REPLY::WATCHING) {
    return true;
  }
  return false;
}
 ...
 */
bool reversi::KeyboardSelectYesNoWatching::CheckInputStringType(
    const std::string& checkString,
    reversi::KeyboardSelectYesNoWatching::SELECT_REPLY& reply) {
  ...
  for (int i = 0; i < YES_UPPER_PATTERN_COUNT; ++i) {
    reversi::Assert::AssertArrayRange(
        i, YES_UPPER_PATTERN_COUNT,
        "KeyboardSelectYesNoWatching::CheckInputStringType index over yes");
  ...
  for (int i = 0; i < NO_UPPER_PATTERN_COUNT; ++i) {
    reversi::Assert::AssertArrayRange(
        i, NO_UPPER_PATTERN_COUNT,
        "KeyboardSelectYesNoWatching::CheckInputStringType index over no");
  ...
  for (int i = 0; i < WATCHING_UPPER_PATTERN_COUNT; ++i) {
    reversi::Assert::AssertArrayRange(
        i, WATCHING_UPPER_PATTERN_COUNT,
        "KeyboardSelectYesNoWatching::CheckInputStringType index over "
        "watching");
 ...
 */
void reversi::KeyboardSelectYesNoWatching::TransformStringUpper(
