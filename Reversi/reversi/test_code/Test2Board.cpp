 ...
 */
test_code::Test2Board::Test2Board(
 ...
 *
 */
void test_code::Test2Board::ExecuteUnitTest() {
  {
    reversi::Board board;
    ...
    for (int i = 0; i < size; ++i) {
      AssertRange(i, size, "TestBoard::Execute() position index over");

      reversi::ReversiConstant::POSITION position =
          (reversi::ReversiConstant::POSITION)
              reversi::ReversiConstant::POSITIONS[i];

      AssertRange((int)position, reversi::ReversiConstant::BOARD_SIZE,
      ...
      if ((position == reversi::ReversiConstant::POSITION::D4) ||
        ...
        // 白の初期位置
        AssertEquals(info == reversi::ReversiConstant::BOARD_INFO::WHITE,
        ...
        // 黒の初期位置
        AssertEquals(info == reversi::ReversiConstant::BOARD_INFO::BLACK,
                     "TestBoard::Execute() NOT BLACK");
      } else {
        // 空白
        AssertEquals(info == reversi::ReversiConstant::BOARD_INFO::NONE,
  ...
  // 裏返し処理のチェック
  {
    ...
    AssertEquals(board.Move(moveInfoClass), "TestBoard::Execute() not move");
  ...
  // 盤面Fullチェック
  {
    ...
    // 盤面が埋まったか
    AssertEquals(board.IsFull(), "TestBoard::Execute() NOT FULL 2");
  }
  // カウント
  {
    reversi::Board board;
