...
class ReversiConstant {
 public:
  ...
  static const int BOARD_SIZE =
      ReversiConstant::BOARD_DATA_SIZE_Y *
      ReversiConstant::
          BOARD_DATA_SIZE_X;  // 盤の大きさ(1次元配列)
                              // 8x8の盤だが斜め判定を考慮して全体を無効マスで囲う用にしている
  ...
  static const int ONE_MOVE_MAX_REVERSE_COUNT =
      18;  // 1回の着手でひっくり返る最大数

  static const ReversiConstant::POSITION POSITIONS
      [ReversiConstant::
           POSITION_SIZE];  // 全ての座標(通常使用するもののみ、番兵は含まない)
  static const ReversiConstant::POSITION
      POSITION_HORIZONTALS[ReversiConstant::BOARD_SIZE_Y]
                          [ReversiConstant::BOARD_SIZE_X];  // 横方向の座標
  static const std::string POSITION_STRINGS
      [ReversiConstant::
           POSITION_SIZE];  // 位置の文字列テーブル(POSITIONSのindexに対応)
 public:
   ...
   * @return          指定座標の上の座標
   */
  static int GetPositionLeft(int position) { return position - 1; }
   ...
   * @return          指定座標の上の座標
   */
  static int GetPositionRight(int position) { return position + 1; }
   ...
   * @return                trueなら変換できた
   */
  static bool GetStringToPosition(const std::string& positionString,
   ...
   * @return                trueなら変換できた
   */
  static bool GetPositionToString(
   ...
   * @return      切り替えされた手番
   */
  static reversi::ReversiConstant::TURN InvertTurn(
