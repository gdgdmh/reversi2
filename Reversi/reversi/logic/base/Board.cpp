...
bool reversi::Board::Move(reversi::MoveInfo moveInfo) {
  ...
  for (int i = 0; i < reversi::ReverseInfo::MAX_DIRECTION; ++i) {
    reversi::Assert::AssertArrayRange(i, reversi::ReverseInfo::MAX_DIRECTION,
                                      "Board::Move index over i");
    int reverseCount =
        reverseInfo.GetReversePositionCount((reversi::ReverseInfo::DIRECTION)i);
    for (int j = 0; j < reverseCount; ++j) {
      reversi::Assert::AssertArrayRange(j, reverseCount,
                                        "Board::Move index over j");
      // 裏返す
      reversi::ReversiConstant::POSITION position =
          reverseInfo.GetReversePosition((reversi::ReverseInfo::DIRECTION)i, j);
 ...
 * @return          盤情報
 */
reversi::ReversiConstant::BOARD_INFO reversi::Board::GetBoardInfo(
    int position) {
  reversi::Assert::AssertArrayRange((int)position,
                                    reversi::ReversiConstant::BOARD_SIZE,
                                    "Board::GetBoardInfo index over");
 ...
 */
void reversi::Board::GetCount(int& blackCount, int& whiteCount,
                              int& noneCount) {
  blackCount = 0;
  whiteCount = 0;
  noneCount = 0;
  ...
  for (int i = 0; i < size; ++i) {
    reversi::Assert::AssertArrayRange(i, size,
                                      "Board::GetCount() position index over");
    // 位置を取得
    reversi::ReversiConstant::POSITION position =
    ...
    reversi::Assert::AssertArrayRange((int)position,
                                      reversi::ReversiConstant::BOARD_SIZE,
                                      "Board::GetCount() board index over");
    reversi::ReversiConstant::BOARD_INFO info =
        (reversi::ReversiConstant::BOARD_INFO)board.boardData[(int)position];
    // それぞれの情報をカウント
...
reversi::Board reversi::Board::Clone() const {
  ...
  for (int i = 0; i < ReversiConstant::BOARD_SIZE; ++i) {
    reversi::Assert::AssertArrayRange(i, reversi::ReversiConstant::BOARD_SIZE,
                                      "Board::Clone() index over");
    dest.SetBoardInfo((reversi::ReversiConstant::BOARD_INFO)board.boardData[i],
                      (reversi::ReversiConstant::POSITION)i);
  }
...
void reversi::Board::Copy(const reversi::Board& source) {
  for (int i = 0; i < ReversiConstant::BOARD_SIZE; ++i) {
    reversi::Assert::AssertArrayRange(i, reversi::ReversiConstant::BOARD_SIZE,
                                      "Board::Clone() index over");
    SetBoardInfo(
        (reversi::ReversiConstant::BOARD_INFO)source.board.boardData[i],
        (reversi::ReversiConstant::POSITION)i);
  }
...
bool reversi::Board::IsFull() const {
  ...
  for (int i = 0; i < size; ++i) {
    reversi::Assert::AssertArrayRange(i, size,
                                      "Board::IsFull() position index over");
    // 位置を取得
    reversi::ReversiConstant::POSITION position =
        (reversi::ReversiConstant::POSITION)
            reversi::ReversiConstant::POSITIONS[i];
...
void reversi::Board::ExecuteFull(reversi::ReversiConstant::BOARD_INFO setInfo) {
  ...
  for (int i = 0; i < size; ++i) {
    reversi::Assert::AssertArrayRange(
        i, size, "Board::ExecuteFull() position index over");
    // 位置を取得
    reversi::ReversiConstant::POSITION position =
        (reversi::ReversiConstant::POSITION)
            reversi::ReversiConstant::POSITIONS[i];
    reversi::Assert::AssertArrayRange((int)position,
                                      reversi::ReversiConstant::BOARD_SIZE,
                                      "Board::ExecuteFull() board index over");
    reversi::ReversiConstant::BOARD_INFO info =
        (reversi::ReversiConstant::BOARD_INFO)board.boardData[(int)position];
    if (info == reversi::ReversiConstant::BOARD_INFO::NONE) {
      // 空マスを埋める
...
void reversi::Board::SetDebugPass() {
  ...
  for (int i = 0; i < BLACK_COUNT; ++i) {
    reversi::Assert::AssertArrayRange(i, BLACK_COUNT,
                                      "Board::SetDebugPass index over");
    SetBoardInfo(reversi::ReversiConstant::BOARD_INFO::BLACK,
                 blackPositions[i]);
  }
  SetBoardInfo(reversi::ReversiConstant::BOARD_INFO::WHITE,
               ReversiConstant::POSITION::H3);
}
...
void reversi::Board::Clear() {
  for (int i = 0; i < reversi::ReversiConstant::BOARD_SIZE; ++i) {
    reversi::Assert::AssertArrayRange(i, reversi::ReversiConstant::BOARD_SIZE,
                                      "Board::Clear() index over");
    SetBoardInfo(reversi::ReversiConstant::BOARD_INFO::INVALID,
                 (reversi::ReversiConstant::POSITION)i);
  }
}
...
void reversi::Board::SetEmpty() {
  for (int i = 0; i < reversi::ReversiConstant::POSITION_SIZE; ++i) {
    reversi::Assert::AssertArrayRange(i,
                                      reversi::ReversiConstant::POSITION_SIZE,
                                      "Board::SetEmpty() index over positions");
    reversi::Assert::AssertArrayRange(
        (int)reversi::ReversiConstant::POSITIONS[i],
        reversi::ReversiConstant::BOARD_SIZE,
        "Board::SetEmpty() index over board");
    SetBoardInfo(reversi::ReversiConstant::BOARD_INFO::NONE,
                 reversi::ReversiConstant::POSITIONS[i]);
  }
}
...
bool reversi::Board::CheckEmpty(reversi::ReversiConstant::POSITION position) {
  reversi::Assert::AssertArrayRange((int)position,
                                    reversi::ReversiConstant::BOARD_SIZE,
                                    "Board::CheckEmpty index over");
  if ((reversi::ReversiConstant::BOARD_INFO)board.boardData[(int)position] ==
      reversi::ReversiConstant::BOARD_INFO::NONE) {
    return true;
  }
  return false;
}
 ...
 * @return      手番に対応した石
 */
reversi::ReversiConstant::BOARD_INFO reversi::Board::GetTurnToStone(
 ...
 */
void reversi::Board::SetBoardInfo(reversi::ReversiConstant::BOARD_INFO info,
                                  reversi::ReversiConstant::POSITION position) {
  reversi::Assert::AssertArrayRange((int)position,
                                    reversi::ReversiConstant::BOARD_SIZE,
                                    "Board::SetBoardInfo index over");
