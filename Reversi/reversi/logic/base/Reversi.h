...
class Reversi {
 ...
 public:
   ...
   * @return 盤クラス
   */
  const reversi::Board& GetBoard() const { return board; }

  /**
   * 結果を取得する
   * @return 結果
   */
  reversi::Reversi::RESULT GetResult() const { return resultData.result; }

  /**
   * 現在の手番を取得する
   * @return 手番
   */
  reversi::ReversiConstant::TURN GetTurn() const { return turn; }
 ...
 private:
   ...
   * @return             作成したプレイヤークラス
   */
  reversi::IPlayer* CreatePlayer(int playerIndex,
                                 reversi::Reversi::PLAYER playerType);
   ...
   * @param result 勝敗結果
   */
  void SetResultStone(int& black, int& white, int& none,
 ...
 private:
  ...
  bool
      checkEnableMove;  // 着手できるかチェック(思考のときは遅くなるのでフラグで管理)
};
