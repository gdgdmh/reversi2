...
bool reversi::MoveThinkingCpu2::MoveThinking(const reversi::Reversi& reversi, const reversi::Move& moveCache, const reversi::Board& board, reversi::MoveInfo& move, reversi::ReversiConstant::TURN turn) {
	...
	if (CheckPutEnableCornerIndex(index, reversiMove)) {
		...
		// 候補0はおかしい
		reversi::Assert::AssertEquals(candidate.size() > 0, "MoveThinkingCpu2::MoveThinking candidate index size 0");
 ...
 * @param move      着手キャッシュ
 */
void reversi::MoveThinkingCpu2::GetMaxReverseCountCandidateIndex(std::vector<int>& candidate, const reversi::Move& move) {
	candidate.clear();
	// 取れる箇所が最大値の場所を探す
	...
	reversi::Assert::AssertEquals(candidate.size() > 0, "MoveThinkingCpu2::GetMaxReverseCountCandidateIndex ReverseInfo not hit");
}
 ...
 * @return             trueなら見つかった
 */
bool reversi::MoveThinkingCpu2::CheckPutEnableCornerIndex(int& cornerIndex, const reversi::Move& move) {
