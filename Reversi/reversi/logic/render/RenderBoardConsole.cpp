...
void reversi::RenderBoardConsole::RenderBoardHorizontalCells(mhl::IOutputConsole* const console, const Board& board, int y) {
	...
	for (int i = 0; i < reversi::ReversiConstant::BOARD_SIZE_X; ++i) {
		...
		// 盤の情報から表示文字列を結合
		out += "　" + GetBoardInfoString((reversi::ReversiConstant::BOARD_INFO)rawData.boardData[position]) + "　｜";
	}
...
std::string reversi::RenderBoardConsole::GetBoardInfoString(reversi::ReversiConstant::BOARD_INFO info) {
	switch (info) {
