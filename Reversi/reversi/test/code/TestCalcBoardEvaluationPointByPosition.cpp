 ...
 * デストラクタ
 */
reversi::TestCalcBoardEvaluationPointByPosition::~TestCalcBoardEvaluationPointByPosition() {
}
...
bool reversi::TestCalcBoardEvaluationPointByPosition::Execute() {
	{
		...
		int black = 0, white = 0;
		calcEval.CalcBoardEvaluationPoint(board, black, white, reversi::ReversiConstant::TURN::TURN_BLACK);
		// 初期配置では現在の評価値ではイコールになる(将来的に評価値の基準が変わったら前提を変更しても可)
		if (!AssertEqual((black == white), "TestCalcBoardEvaluationPointByPosition::Execute NOT initialize position equal")) {
			return false;
		}
	}
	{
		...
		if (move.CheckEnableMoveByCache(position)) {
			...
			if (!AssertEqual((black > white), "TestCalcBoardEvaluationPointByPosition::Execute NOT eval up")) {
				return false;
			}
		} else {
			// 本来はこれはMoveのテストで検出されるべきだが一応
			if (!AssertEqual(0, "TestCalcBoardEvaluationPointByPosition::Execute NOT move")) {
				return false;
			}
		}
	}
