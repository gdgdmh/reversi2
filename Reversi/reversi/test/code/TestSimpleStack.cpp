 ...
 * コンストラクタ
 */
reversi::TestSimpleStack::TestSimpleStack(mhl::IOutputConsole* outputConsole)
...
bool reversi::TestSimpleStack::Execute() {
  // pushとpopの値とサイズをチェック
  {
    ...
    if (!AssertEqual(stack.GetSize() == STACK_SIZE,
                     "TestSimpleStack::Execute size invalid")) {
      return false;
    }
    // 最後尾データをチェック
    if (!AssertEqual(stack.Back() == (STACK_SIZE - 1),
    ...
    for (int i = (STACK_SIZE - 1); i >= 0; --i) {
      ...
      if (!AssertEqual(data == i,
                       "TestSimpleStack::Execute pop value invalid")) {
        return false;
      }
      int stackSize = stack.GetSize();
      if (!AssertEqual(stackSize == i,
