 ...
 * コンストラクタ
 */
reversi::TestReverseInfo::TestReverseInfo(mhl::IOutputConsole* outputConsole)
...
bool reversi::TestReverseInfo::Execute() {
  ...
  AssertEqual(reversi::ReversiConstant::POSITION::B4 == info.GetPosition(),
              "TestReverseInfo::Execute invalid position");
  AssertEqual(reversi::ReversiConstant::TURN::TURN_BLACK == info.GetTurn(),
  ...
  // 方向のデータ数を確認
  if (!AssertEqual(info.GetReversePositionCount(
                       reversi::ReverseInfo::DIRECTION::UP) == 2,
                   "TestReverseInfo::Execute invalid up count")) {
    return false;
  }
  if (!AssertEqual(info.GetReversePositionCount(
                       reversi::ReverseInfo::DIRECTION::DOWN) == 1,
  ...
  if (!AssertEqual(
          info.GetReversePosition(reversi::ReverseInfo::DIRECTION::DOWN_LEFT,
                                  0) == reversi::ReversiConstant::POSITION::A3,
