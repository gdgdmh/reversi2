 ...
 * コンストラクタ
 */
reversi::TestThinkingNode::TestThinkingNode(mhl::IOutputConsole* outputConsole)
...
bool reversi::TestThinkingNode::Execute() {
  ...
  reversi::ThinkingNode root;
  {
    ...
    if (!AssertEqual(parent == root.GetParent(),
                     "TestThinkingNode::Execute invalid parent")) {
      return false;
    }
    if (!AssertEqual(position == root.GetMovePosition(),
                     "TestThinkingNode::Execute invalid movePositon")) {
      return false;
    }
    if (!AssertEqual(turn == root.GetTurn(),
                     "TestThinkingNode::Execute invalid turn")) {
      return false;
    }
    if (!AssertEqual(evalPoint == root.GetEvaluationPoint(),
                     "TestThinkingNode::Execute invalid eval point")) {
      return false;
    }
    if (!AssertEqual(thinkDepth == root.GetThinkingDepth(),
                     "TestThinkingNode::Execute invalid thinking depth")) {
      return false;
    }
    if (!AssertEqual(visited == root.GetVisited(),
  ...
  // 最初のchildの下にchildを登録
  {
    ...
    if (!AssertEqual(root.GetChildSize() == 2,
    ...
    if (!AssertEqual(
            root.GetChild(0)->GetChildSize() == 1,
  ...
  // 評価値検索
  {
    ...
    if (!AssertEqual(highNode->GetEvaluationPoint() == 13,
  ...
  // 評価値検索(rootの1つ下のNodeの中で一番評価値が高いノードを探す)
  {
    ...
    // 2つめのchildが選ばれるはず
    if (!AssertEqual(highNode == root.GetChild(1),
