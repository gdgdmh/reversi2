 ...
 * コンストラクタ
 */
reversi::TestMove::TestMove(mhl::IOutputConsole* outputConsole)
...
bool reversi::TestMove::Execute() {
...

  {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      if (!AssertEqual(
              (position != reversi::ReversiConstant::POSITION::D4) &&
...

    {
      ...
      for (int i = 0; i < size; ++i) {
        ...
        switch (position) {
          ...
          case reversi::ReversiConstant::POSITION::F5:
          case reversi::ReversiConstant::POSITION::E6:
            if (!AssertEqual(move.CheckEnableMoveByCache(position),
