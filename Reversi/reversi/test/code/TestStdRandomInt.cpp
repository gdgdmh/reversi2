 ...
 * コンストラクタ
 */
reversi::TestStdRandomInt::TestStdRandomInt(mhl::IOutputConsole* outputConsole)
...
bool reversi::TestStdRandomInt::Execute() {
  ...
  // 同じ乱数の種を使うと同じ値が返ってくるはず
  {
    ...
    if (!AssertEqual(result == result2,
                     "TestStdRandomInt::Execute NOT SAME VALUE")) {
      if (random) {
        delete random;
        random = NULL;
      }
  ...
  for (int i = 0; i < CHECK_COUNT; ++i) {
    ...
    if (!AssertEqual(
            (result >= MIN_RANDOM_VALUE) && (result <= MAX_RANDOM_VALUE),
