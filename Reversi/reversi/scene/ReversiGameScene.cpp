...
reversi::ReversiGameScene::ReversiGameScene()
    : console(NULL),
      scene(reversi::ReversiGameScene::SCENE::INITIALIZE),
...
void reversi::ReversiGameScene::TaskInitialize() {
  ...
  console->PrintLine(
      "黒石でプレイする場合はYes 白石でプレイする場合はNo "
      "CPU対局観戦する場合はWatching を入力してEnterを押してください");
...
void reversi::ReversiGameScene::TaskAskPlayerSetting() {
  ...
  if (selectYesNoWatching->IsSelected()) {
    console->PrintLine(
        "CPUの強さを数字で選択してください、数値が高いほど強いです 1 or 2 or 3 "
        "or 4");
...
void reversi::ReversiGameScene::TaskAskCpuSetting() {
  ...
  if (selectCpuLevel->IsSelected()) {
    ...
    if (selectYesNoWatching->IsSelectWatching()) {
      console->PrintLine(
          "2人目のCPUの強さを数字で選択してください、数値が高いほど強いです 1 "
          "or 2 or 3 or 4");
...
void reversi::ReversiGameScene::TaskReversiStart() {
  ...
  if (selectYesNoWatching->IsSelectYes()) {
    ...
    setting.playerType[reversi::Reversi::PLAYER_BLACK] =
        reversi::Reversi::PLAYER::MAN;
    setting.playerType[reversi::Reversi::PLAYER_WHITE] =
        NumberToCpuLevel(selectCpuLevel->GetCpuLevel());
    ...
    setting.playerType[reversi::Reversi::PLAYER_BLACK] =
        NumberToCpuLevel(selectCpuLevel->GetCpuLevel());
    setting.playerType[reversi::Reversi::PLAYER_WHITE] =
        NumberToCpuLevel(selectCpuLevel2->GetCpuLevel());
  }
  // リバーシロジックの初期化
  reversi.Initialize();
 ...
 */
void reversi::ReversiGameScene::SetScene(
    reversi::ReversiGameScene::SCENE nextScene) {
  scene = nextScene;
}
 ...
 */
reversi::Reversi::PLAYER reversi::ReversiGameScene::NumberToCpuLevel(
    int cpuLevel) {
  switch (cpuLevel) {
    case 1:
...
bool reversi::ReversiGameScene::IsPlayerWin() {
  ...
  if (playerSetting.playerType[reversi::Reversi::PLAYER_BLACK] ==
      reversi::Reversi::PLAYER::MAN) {
  ...
  if (playerSetting.playerType[reversi::Reversi::PLAYER_WHITE] ==
      reversi::Reversi::PLAYER::MAN) {
...
bool reversi::ReversiGameScene::IsPlayerDraw() {
  ...
  if ((playerSetting.playerType[reversi::Reversi::PLAYER_BLACK] ==
       reversi::Reversi::PLAYER::MAN) ||
      (playerSetting.playerType[reversi::Reversi::PLAYER_WHITE] ==
       reversi::Reversi::PLAYER::MAN)) {
